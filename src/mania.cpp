...
bool init() {
    ...
    // importing sprites
    left_handup.setTexture(data::load_texture("files/mania/leftup.png"));
    right_handup.setTexture(data::load_texture("files/mania/rightup.png"));
    for (int i = 0; i < 3; i++) {
        left_hand[i].setTexture(data::load_texture("files/mania/left" + std::to_string(i) + ".png"));
        right_hand[i].setTexture(data::load_texture("files/mania/right" + std::to_string(i) + ".png"));
    }

    if (is_4K) {
        bg.setTexture(data::load_texture("files/mania/4K/bg.png"));
        for (int i = 0; i < 2; i++) {
            left_4K[i].setTexture(data::load_texture("files/mania/4K/" + std::to_string(i) + ".png"));
        }
        for (int i = 0; i < 2; i++) {
            right_4K[i].setTexture(data::load_texture("files/mania/4K/" + std::to_string(i + 2) + ".png"));
        }
    } else {
        bg.setTexture(data::load_texture("files/mania/7K/bg.png"));
        for (int i = 0; i < 4; i++) {
            left_7K[i].setTexture(data::load_texture("files/mania/7K/" + std::to_string(i) + ".png"));
        }
        for (int i = 0; i < 4; i++) {
            right_7K[i].setTexture(data::load_texture("files/mania/7K/" + std::to_string(i + 3) + ".png"));
        }
    }
